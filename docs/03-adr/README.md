...
- A trade-off between two viable designs where the loser is not
  obviously bad (e.g. agent architecture, LLM protocol support,
  tool-use loop design).
- A constraint that the code relies on but does not test (e.g. an
  invariant about which services share a session file).
- A reversal of a previous decision — the old ADR is marked
  `Superseded` and the new one explains why.
- A policy that governs how the project is operated (e.g. the zero
  AI-slop contribution policy, blast-radius tiering used by CODEOWNERS).
...
- File name kebab-case, derived from the title: `07-delegate-mode-architecture.md`.
...
| # | Title | Status |
|---|-------|--------|
| [01](01-deterministic-agent-hybrid.md) | Deterministic engineering x agent hybrid architecture | Accepted |
| [02](02-multi-protocol-llm-client.md) | Multi-protocol LLM client abstraction | Accepted |
| [03](03-tool-use-over-generic-prompts.md) | Tool-use agent over generic prompt-only review | Accepted |
| [04](04-session-resume-identity.md) | Session resume via identity fingerprinting | Accepted |
| [05](05-github-action-composite.md) | GitHub Action as a composite action | Accepted |
| [06](06-zero-ai-slop-policy.md) | Zero AI-slop policy — the 100% quality bar | Accepted |
| [07](07-delegate-mode-architecture.md) | Delegate mode — host-agent review without own LLM key | Accepted |
