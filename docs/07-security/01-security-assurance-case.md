...
- **Related:** [02-security-policy.md](02-security-policy.md), [03-trust-boundaries.md](03-trust-boundaries.md)

## Context

OpenCodeReview (OCR) is a CLI tool that reads git diff output from a
local repository, sends code diffs to a configured LLM provider via
HTTPS, receives review comments, and optionally serves a local web
viewer. Because it handles credentials, executes git subprocesses, and
exposes a local HTTP server, it must justify that its security
requirements are met through secure design principles and
countermeasures against common implementation weaknesses.

## Decision

We maintain a public security assurance case that maps threat actors,
trust boundaries, secure design principles, and OWASP/CWE
countermeasures onto the implementation, and we verify it continuously
with automated checks (`go vet`, `govulncheck`, `go test -race`,
Dependabot).
...
OCR performs the following operations:

1. Reads git diff output from a local repository.
2. Sends code diffs to a configured LLM provider (OpenAI, Anthropic,
   etc.) via HTTPS.
3. Receives review comments from the LLM and presents them to the user.
4. Optionally serves a local web viewer for browsing review session
   history.
...
1. **Git → CLI**: Diff content may contain crafted payloads. Parsed with
   strict format validation.
2. **CLI → LLM API**: API keys transmitted over HTTPS only. Responses
   validated before use.
3. **CLI → Local output**: File writes constrained to the working
   directory.
4. **Browser → Viewer**: Host-header allowlist enforces access control;
...
The following analysis maps [Saltzer & Schroeder's design
principles](https://ieeexplore.ieee.org/document/1451869) to the
...
| **Complete mediation** | Every viewer HTTP request is checked against the host allowlist (`internal/viewer/hostguard.go`). Every file path from agent tools is validated against the repository root (`internal/tool/filereader.go`, `internal/pathutil/path.go`). |
...
- **Easier:** reviewers and users can audit the security posture from a
  single document; CI enforces the claims automatically.
- **Harder:** every new feature must be checked against the threat
  model before landing.
- **Given up:** none — the assurance case is documentation, not a
  constraint on the design.

## Related

- [Security policy](02-security-policy.md)
- [Trust boundaries](03-trust-boundaries.md)
