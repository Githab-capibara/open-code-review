...
core command documented in the [CLI Reference](../user-guide/04-cli-reference.md#json).
...
4. **Run the review in range mode** with machine-readable output, so
   ...
   [JSON output](../user-guide/04-cli-reference.md#json) for the envelope every
   recipe consumes.
...
[tips that apply to every pattern](#background-context).
...
See [Review Rules](../user-guide/05-review-rules.md) for the schema.
...
See [Review Rules](../user-guide/05-review-rules.md) for the rule schema.
...
- [CLI Reference](../user-guide/04-cli-reference.md#json) — the JSON output shape both pipelines
  consume, useful when writing your own CI script from scratch.
- [Configuration](../user-guide/03-configuration.md) — every env var and config
  key OCR honors.
